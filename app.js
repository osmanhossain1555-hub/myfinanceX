...
function renderStats() {
  ...
  const loans = sum(state.entries.filter(e => e.type === 'loan').map(e => e.amount));
  const lends = sum(state.entries.filter(e => e.type === 'lend').map(e => e.amount));
  ...
  const needs = sum(state.entries.filter(e => e.type === 'expense' && /need|rent|bill|grocery|home/i.test(e.category)).map(e => e.amount));
  const wants = sum(state.entries.filter(e => e.type === 'expense' && /want|shop|fun|entertain|travel/i.test(e.category)).map(e => e.amount));
...
function renderTrend() {
  const last7 = Array.from({ length: 7 }, (_, i) => {
    ...
    return sum(dayEntries.map(e => (e.type === 'income' ? e.amount : -e.amount)));
  });
  ...
  els.trendChart.innerHTML = `
    ...
    ${last7.map((v, i) => {
      ...
      return `<circle cx="${x}" cy="${y}" r="4" fill="#4d67ff" stroke="#ffffff" stroke-width="2" />`;
    }).join('')}
  `;
...
function money(v) { return new Intl.NumberFormat(undefined, { style: 'currency', currency: 'USD' }).format(v || 0); }
